...
class InstallerController extends MelisAbstractActionController
{
    ...
    public function indexAction()
    {
        ...
        $resources = $this->getServiceManager()->get('MelisAssetManagerWebPack')->getAssets();
        $locales = $this->getTranslationsLocale();


        $routeMatch = $this->getServiceManager()->get('Application')->getMvcEvent();
    ...
    {
        $modulesSvc = $this->getServiceManager()->get('MelisAssetManagerModulesService');
    ...
    {
        $form = null;
        $melisConfig = $this->getServiceManager()->get('MelisInstallerConfig');
    ...
    public function newEnvironmentAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            for ($x = 0; $x <= count($data) / 2; $x++) {
                ...
                if (
                    isset($data[$environmentName]) && isset($data[$domainName]) &&
...
//                    $siteModulePath = 'melisplatform/melis-platform-framework-' . $frameworkName . '-demo-site';
    ...
    public function activateModulesAction()
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            // drop melis-site packages (extra.melis-site = true) - they belong in
            // module/MelisSites and must never be written to the back-office module loader
            $loadableModules = $moduleSvc->getAllModules(); // getVendorModules() already excludes melis-site packages
            $modules = array_values(array_filter($modules, function ($module) use ($loadableModules, $autoInstallModules) {
                return in_array($module, $loadableModules) || in_array($module, $autoInstallModules);
            }));
    ...
    {
        $installSvc = $this->getServiceManager()->get('InstallerHelper');
        $result = [];
        ...
        foreach ($modules as $module) {
            ...
            if (!in_array($module, $modulesExclusions)) {
                ...
                if ($moduleFormContent) {
                    ...
                    $tabs .= '<li class="nav-item"><a class="nav-link ' . $active . '" href="#' . $id . '" data-bs-toggle="tab">' . $module . '</a></li>';
    ...
    protected function marketplace()
    {
        ...
        $marketplace = $this->getServiceManager()->get('MelisMarketPlaceService');
    ...
    protected function marketplaceSite()
    {
        ...
        $marketplaceSite = $this->getServiceManager()->get('MelisMarketPlaceSiteService');
